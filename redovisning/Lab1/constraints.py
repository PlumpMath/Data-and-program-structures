...
class Squarer:
	...
	def process(self):
		m1, prod = self.m1, self.prod
		if m1.has_value() and m1 == 0:
			prod.set_value(0,self)
		elif m1.has_value():
			prod.set_value(m1.value * m1.value , self)
		elif prod.has_value() and prod.value == 0:
			m1.setvalue(0,self)
		elif prod.has_value():
			m1.setvalue(math.sqrt(prod.value),self)
	def forget(self):
		self.prod.forget(self)
		self.m1.forget(self)
		self.process()
...
# 3.0.1 b) Ideally we'd want functions with inverse functions where
#          both the function and its inverse has singular solutions.
